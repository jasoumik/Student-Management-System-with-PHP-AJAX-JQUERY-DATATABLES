...
if(isset($_POST["action"]))
{
 if($_POST["action"] == 'fetch')
 {
  ...
  $output = array(
   ...
   "recordsFiltered" => get_total_records($connect, 'grade_table'),
   "data"    => $data
  );
 ...
 if($_POST["action"] == 'Add' || $_POST["action"] == "Edit")
 {
  ...
  else
  {
   if($_POST["action"] == "Add")
   {
    ...
    $query = "INSERT INTO grade_table 
    (grade_name) 
    SELECT * FROM (SELECT :grade_name) as temp 
    WHERE NOT EXISTS (
     SELECT grade_name FROM grade_table WHERE grade_name = :grade_name
    ) LIMIT 1
    ";
    $statement = $connect->prepare($query);
 ...
 {
  $query = "SELECT * FROM grade_table WHERE grade_id = '".$_POST["grade_id"]."'";
  $statement = $connect->prepare($query);
 ...
 {
  $query = "DELETE FROM grade_table WHERE grade_id = '".$_POST["grade_id"]."'";
  $statement = $connect->prepare($query);
  if($statement->execute())
  {
